...
このハーネスは、単なるCLAUDE.mdファイルではなく、完全な開発システムをプロジェクトにインストールします——**15の専門エージェント、33のスキル**、そしてエージェントに**理解してからビルドする**ことを強制するフェーズロック型ワークフローです。
...
| スキルシステム | 33の自動検出、プロンプトごとにキーワードマッチ | 86の合成可能スキル | メタプロンプティング | 23のロールツール |
...
リクエストをルーティングする前に、<strong>33のスキル</strong>をスキャンしてキーワードをマッチング。「add webhook support」と言えば、<code>vc-security</code> + <code>vc-scenario</code>が自動的にサーフェスされます。
</td>
<td width="50%" valign="top">
<h1>💾</h1>
...
| 📝 **SPEC** | ユーザーレビュー用のプロダクトディスカバリー要件ドキュメント | `go` または `ENTER SPEC MODE` |
| 💡 **INNOVATE** | トレードオフ付きで2〜3のアプローチを探索 | `go` |
| 📋 **PLAN** | レビュー可能な詳細スペックを作成 | `go` |
| ✅ **VALIDATE** | プランを実行可能なコントラクトに変換；ゲートが通過する必要がある | `ENTER VALIDATE MODE` |
...
│   ├── backlog/                 # 📌 保留中の作業
│   ├── reports/                 # 📄 （非推奨——成果物はタスクフォルダー内に置く）
│   └── references/              # 📚 （非推奨——成果物はタスクフォルダー内に置く）
└── features/
...
<details>
<summary>クリックでエージェント一覧を展開（15エージェント）</summary>
...
<details>
<summary>クリックでスキル一覧を展開（33スキル）</summary>

<br>

**🔧 コントラクト** — `vc-generate-plan` · `vc-generate-context` · `vc-generate-spec` · `vc-generate-closeout` · `vc-generate-phase-program` · `vc-audit-context` · `vc-audit-plans` · `vc-audit-vc` · `vc-setup` · `vc-update` · `vc-publish`

**🧠 プランニング＆バリデーション** — `vc-predict` · `vc-scenario` · `vc-sequential-thinking` · `vc-problem-solving` · `vc-feasibility-test` · `vc-risk-evidence-pack` · `vc-test-coverage-plan` · `vc-validate-findings` · `vc-agent-strategy-compare` · `vc-intent-clarify` · `vc-plan-discovery` · `vc-review-situation`

**🐛 デバッグ＆セキュリティ** — `vc-debug` · `vc-security` · `vc-autoresearch`

**📚 リサーチ** — `vc-docs-seeker` · `vc-scout`

**🎨 フロントエンド** — `vc-frontend-design` · `vc-agent-browser` · `vc-web-testing`

**⚙️ オートメーション** — `vc-autopilot` · `vc-context-discovery`
...
| 📋 **プラン構造チェック** (`post-write-plan-check.mjs`) | `*_PLAN_*.md`ファイルへの書き込み時にプランアーティファクト構造を検証 |
| 🧹 **コミットメッセージリント** (`post-commit-lint.mjs`) | `git commit` Bash呼び出し時にconventional-commitsプレフィックスをチェック |
| 🔍 **停止バリデータースイープ** (`stop-validator-sweep.cjs`) | エージェントセッション停止時にコアフレームワークバリデーターを実行 |
...
│   ├── skills/              # ⚡ 33のスキルモジュール（各ディレクトリにSKILL.md）
│   └── hooks/               # 🪝 10つのライフサイクルフック（.cjs and .mjs）
├── .codex/
│   └── agents/              # 🔄 Codex互換ミラー
├── .agents/
