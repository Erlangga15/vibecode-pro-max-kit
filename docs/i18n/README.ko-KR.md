...
your-project/
├── .claude/
│   ├── agents/              # 🤖 15개의 전문 에이전트 정의
│   │   ├── vc-research-agent.md
│   │   ├── vc-execute-agent.md
│   │   └── ...
│   ├── skills/              # ⚡ 33개의 자동 탐색 skill
│   │   ├── vc-generate-plan/
│   │   ├── vc-security/
│   │   ├── vc-scout/
│   │   └── ...
│   └── hooks/               # 🪝 10개의 라이프사이클 hook
│       ├── privacy-block.cjs
│       ├── scout-block.cjs
│       └── ...
├── .codex/
...
이 harness는 프로젝트에 완전한 개발 시스템을 설치해요 — 그냥 CLAUDE.md 파일 하나가 아니라, **15개의 전문 에이전트, 33개의 skill**, 그리고 에이전트가 **만들기 전에 이해하도록 강제하는** phase-locked 워크플로우예요.
...
| Skill 시스템 | 33개 자동 탐색, 모든 프롬프트에서 키워드 매칭 | 86개 조합형 skill | 메타 프롬프팅 | 23개 역할 도구 |
...
요청을 라우팅하기 전에 <strong>33개 skill</strong>을 스캔하고 키워드를 매칭해요. "add webhook support"라고 하면 <code>vc-security</code> + <code>vc-scenario</code>가 자동으로 표시돼요.
</td>
<td width="50%" valign="top">
<h1>💾</h1>
...
| 📝 **SPEC** | 사용자 리뷰를 위한 프로덕트 디스커버리 요구사항 문서 | `go` 또는 `ENTER SPEC MODE` |
| 💡 **INNOVATE** | 트레이드오프와 함께 2-3가지 접근 방식 탐색 | `go` |
| 📋 **PLAN** | 리뷰할 수 있는 상세 스펙 작성 | `go` |
| ✅ **VALIDATE** | 플랜을 실행 가능한 컨트랙트로 변환; 게이트가 통과해야 함 | `ENTER VALIDATE MODE` |
...
│   ├── backlog/                 # 📌 지연된 작업
│   ├── reports/                 # 📄 (사용 중단 — 산출물은 태스크 폴더 내에 둬요)
│   └── references/              # 📚 (사용 중단 — 산출물은 태스크 폴더 내에 둬요)
└── features/
...
<details>
<summary>에이전트 목록 펼치기 (15개)</summary>
...
<details>
<summary>Skill 목록 펼치기 (33개)</summary>
...
**🧠 Planning & validation** — `vc-predict` · `vc-scenario` · `vc-sequential-thinking` · `vc-problem-solving` · `vc-feasibility-test` · `vc-risk-evidence-pack` · `vc-test-coverage-plan` · `vc-validate-findings` · `vc-agent-strategy-compare` · `vc-intent-clarify` · `vc-plan-discovery` · `vc-review-situation`

**🐛 Debug & security** — `vc-debug` · `vc-security` · `vc-autoresearch`

**📚 Research** — `vc-docs-seeker` · `vc-scout`

**🎨 Frontend** — `vc-frontend-design` · `vc-agent-browser` · `vc-web-testing`

**⚙️ 자동화** — `vc-autopilot` · `vc-context-discovery`
...
| 📋 **Plan structure check** (`post-write-plan-check.mjs`) | `*_PLAN_*.md` 파일에 쓸 때마다 플랜 산출물 구조를 검증해요 |
| 🧹 **Commit message lint** (`post-commit-lint.mjs`) | `git commit` Bash 호출 시 conventional-commits 접두사를 확인해요 |
| 🔍 **Stop validator sweep** (`stop-validator-sweep.cjs`) | 에이전트 세션이 중지될 때 핵심 프레임워크 검증기를 실행해요 |
...
│   ├── skills/              # ⚡ 33개 skill 모듈 (각각 SKILL.md가 있는 디렉토리)
│   └── hooks/               # 🪝 10개 라이프사이클 hook (.cjs and .mjs)
├── .codex/
